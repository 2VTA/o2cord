...
# baked in (not a placeholder), re-patches Discord Canary if it auto-updated
# since the last patch, THEN syncs the 4 live dist files into the installed
# o2cord/dist folder, then fully restarts Discord Canary with
...
Write-Host "== Stopping Discord Canary ==" -ForegroundColor Cyan
...
if (-not $IsPatched) {
    ...
    & $Installer --install --target=canary
...
$SquirrelLog = Join-Path $CanaryRoot "SquirrelSetup.log"
$LatestApp = $null
if (Test-Path $SquirrelLog) {
    $lastLaunchLine = Get-Content -LiteralPath $SquirrelLog -Tail 50 |
        Where-Object { $_ -match "About to launch: '(.+DiscordCanary\.exe)'" } |
        Select-Object -Last 1
    if ($lastLaunchLine -match "About to launch: '(.+)\\DiscordCanary\.exe'") {
        $candidate = $Matches[1]
        if (Test-Path (Join-Path $candidate "DiscordCanary.exe")) {
...
Stop-Process -Name DiscordCanary -Force -ErrorAction SilentlyContinue
for ($i = 0; $i -lt 10; $i++) {
    if (-not (Get-Process -Name DiscordCanary -ErrorAction SilentlyContinue)) { break }
    Start-Sleep -Seconds 1
}
if (Get-Process -Name DiscordCanary -ErrorAction SilentlyContinue) {
    throw "Discord Canary is still running after 10s - can't safely sync dist files."
}
...
$Exe = Join-Path $LatestApp.FullName "DiscordCanary.exe"
Start-Process -FilePath $Exe -ArgumentList "--remote-debugging-port=9222"
Write-Host "Done. Discord Canary is restarting with the fresh build." -ForegroundColor Green
