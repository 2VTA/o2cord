...
const CHANGELOG_HISTORY_KEY = "O2cordChangelog_History";
const LAST_SEEN_HASH_KEY = "O2cordChangelog_LastSeenHash";
const KNOWN_PLUGINS_KEY = "O2cordChangelog_KnownPlugins";
const KNOWN_SETTINGS_KEY = "O2cordChangelog_KnownSettings";
const LAST_REPO_CHECK_KEY = "O2cordChangelog_LastRepoCheck";
const GITHUB_COMPARE_ENDPOINT = "https://api.github.com/repos";

// o2cord's build hash is a manifest-publish tag (e.g. "public-20260822073010"),
// not a git commit SHA, and its update source is a manifest.json URL rather than
// a github.com repo URL. GitHub's compare API needs real commit SHAs against an
// owner/repo slug, so we point it at the actual repo directly instead of trying
// to derive one from the manifest URL - this only affects the retroactive
// "what changed since my last update" lookup; the primary pending-update flow
// below reads o2cord's own manifest message directly and doesn't need this at all.
const O2CORD_REPO_SLUG = "2VTA/o2cord";
...
export async function getCommitsSinceLastSeen(): Promise<ChangelogEntry[]> {
    const lastSeenHash = await getLastSeenHash();
    if (!lastSeenHash || lastSeenHash === "unknown" || lastSeenHash === gitHash)
        return [];

    return fetchCommitsBetween(O2CORD_REPO_SLUG, lastSeenHash, gitHash);
}
...
export async function getUpdatedPlugins(): Promise<string[]> {
    // Placeholder - o2cord doesn't track per-plugin version changes yet.
    return [];
}
