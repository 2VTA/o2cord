...
function FakeVoiceHeaderButton() {
    const [, forceUpdate] = React.useReducer((x: number) => x + 1, 0);
    return (
        <HeaderBarButton
            onClick={() => {
            ...
            tooltip={isGhostActive ? "Disable Fake Voice" : "Enable Fake Voice (Right click: Config)"}
            icon={FakeDeafenIcon}
            selected={isGhostActive}
            aria-label="Fake Voice"
        />
    );
}
...
export default definePlugin({
    ...
    stop() {
        removeHeaderBarButton("fake-voice-option");
    }
});
